...
class DietCalculatorService
{
    ...
    public function calculateMerFromScalars(
        ...
        // Special physiological status: apply the factor directly to RER and return.
        // Target weight adjustments (weight_loss, weight_gain) are also calculated directly on RER.
        ...
        // "Normal Adult" status: apply activity level factor,
        // differentiating between intact vs neutered for low and medium activity.
        ...
        $mer = $rer * $factor;
        // Adjustment for senior age (> 7 years): -20%
        if ($ageYears > 7) {
            $mer *= 0.80;
        }
    ...
    private function scaleDynamicRecipe(array $recipe, array $targets, float $weightKg): ?array
    {
        ...
        // Fiber (vegetables) should scale with the dog's size/requirement.
        // A standard rule in homemade diets is ~10g of vegetables per 100 kcal of MER.
